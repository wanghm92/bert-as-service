...
class BertSink(threading.Thread):
    ...
    def run(self):
        ...
        while not self.exit_flag.is_set():
            ...
            client_id = msg[0]
            # parsing the ndarray
            arr_info, arr_val_x, arr_val_y = jsonapi.loads(msg[2]), msg[4],  msg[5]
            X = np.frombuffer(memoryview(arr_val_x), dtype=arr_info['dtype_x'])
            X = X.reshape(arr_info['shape_x'])
            Y = np.frombuffer(memoryview(arr_val_y), dtype=arr_info['dtype_y'])
            Y = Y.reshape(arr_info['shape_y'])

            client_info = client_id.split(b'@')
            client_id = client_info[0]
            partial_id = client_info[1] if len(client_info) == 2 else 0
            pending_client[client_id].append((X, Y, partial_id))
            ...
            for client, tmp in finished:
                ...
                tmp_x = [x[0] for x in sorted(tmp, key=lambda x: x[-1])]
                tmp_y = [x[1] for x in sorted(tmp, key=lambda x: x[-1])]
                send_ndarray(self.frontend, client, np.concatenate(tmp_x, axis=0), np.concatenate(tmp_y, axis=0))
...
class BertWorker(Process):
    def __init__(self, id, args, worker_address, sink_address):
        ...
        self.checkpoint_fp = os.path.join(self.bert_model_dir, 'bert_model.ckpt')
        self.vocab_fp = os.path.join(args.bert_model_dir, 'vocab.txt')
    ...
    def input_fn_builder(self, worker):
            ...
            while not self.exit_flag.is_set():
                ...
                if BertClient.is_valid_input(msg):
                    ...
                    yield {
                        ...
                        'label_ids': [f.label_ids for f in tmp_f],
                        'original_token_mask': [f.original_token_mask for f in tmp_f],
                    }
                else:
        ...
        def input_fn():
            '''
             The elements generated by generator must be compatible
             with the given output_types and (optional) output_shapes arguments.
            '''
            return (tf.data.Dataset.from_generator(
                ...
                output_shapes={
                    ...
                    'label_ids': (None, self.max_seq_len),
                    'segment_ids': (None, self.max_seq_len),
                    'original_token_mask': (None, self.max_seq_len),
                }))

        return input_fn


def send_ndarray(src, dest, X, Y, flags=0, copy=True, track=False):
    """send a numpy array with metadata"""
    md = dict(dtype_x=str(X.dtype), shape_x=X.shape, dtype_y=str(Y.dtype), shape_y=Y.shape)
    print("md = {}".format(md))
    return src.send_multipart([dest, b'', jsonapi.dumps(md), b'', X, Y],
